...
model = PeftModel.from_pretrained(model, './llama-2-7b-py-full-trans-model')
...
for example in test_set:
    ...
    if len(prompt) < 500 and prompt != '' and ref != None and ref != '': # 4096 for 200
        t.append(example)
...
for example in test_set:
    ...
    if len(test_set) / 8 <= count:
        ...
        count = 0
'''
with open('/scratch/jotten4/python_translation/llm-small_test_translations.json','w') as outfile:
